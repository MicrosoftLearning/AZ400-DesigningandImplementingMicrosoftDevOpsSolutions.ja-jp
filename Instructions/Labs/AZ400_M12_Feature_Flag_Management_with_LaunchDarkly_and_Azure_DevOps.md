...
1.  運用環境の SDK キーを記録します。これは、このラボで後ほど必要になります。
...
1.  Visual Studio ウィンドウ内の「**Git の変更**」ペインの最上部にある**マスター** ドロップダウン リストで、下向き矢印をクリックします。ドロップダウン ダイアログ ボックスで「**リモート**」をクリックします。リモート ブランチのリストで「**origin/launch-darkly**」を選択します。**「チェック アウト」** を選択します。 
...
1.  **チーム エクスプローラー** ペインの**ホーム** ページで、「**作業項目**」をクリックします。プロンプトが表示されたら、新しいハブを表示するためのリンクを選択します。
...
1.  Visual Studio ウィンドウに切り替え、「**Git 変更**」ペインにi移動し、「**メッセージの入力**」テキストボックスに「**Integated LaunchDarkly #<work\_item\_ID>**」と入力します。ここで、**<work\_item\_ID>** は以前にメモしておいた ID になります。「**すべてコミット**」ボタンの隣にある下向き矢印をクリックし、ドロップダウン リストで「**すべてをコミットしてプッシュ**」をクリックします。**すべてコミット**がグレイアウトされている場合、上矢印を選択して、プッシュを実行できます。 
