...
1. Azure Portal で、(検索バーの横にある) **Cloud Shell** を開きます。
    ...
    > **注**: Azure portal からストレージの作成を求められた場合は、**[ストレージ アカウントは必要ありません]** オプションを選択し、ご自分のサブスクリプションを選択して **[適用]** をクリックします 
...
1. 右上にある **[変更をコミットする...]** をクリックし、**[変更をコミットする]** を既定値 (メイン ブランチを変更する) のままにします。 ワークフローが自動的に実行されます。
