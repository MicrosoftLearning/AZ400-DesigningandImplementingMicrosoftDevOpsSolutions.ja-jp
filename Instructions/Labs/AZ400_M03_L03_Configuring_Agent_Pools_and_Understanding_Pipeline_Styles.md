...
2. **[パイプラインの作成]** ボタンをクリックします。まだ他のパイプラインを作成していない場合は、**[新しいパイプライン]** をクリックして新しいパイプラインを作成します。 
...
19. プロジェクトの一覧から、**EShopOnWeb** プロジェクトを表すタイルをクリックします。
...
22. **[EShopOnWeb]** 編集ペインの既存の YAML ベースのパイプラインで、ターゲット エージェント プールを指定している 13 行目の `vmImage: windows-latest` の行を以下の内容に置き換えて、新しく作成したセルフホステッド エージェント プールを指定します。
...
23. **EShopOnWeb** の編集ペインの右上隅にある **[保存して実行]** をクリックします。 これにより、このパイプラインに基づいてビルドが自動的にトリガーされます。
24. Azure DevOps ポータルの左側の垂直ナビゲーション ペインの [**パイプライン**] セクションで、[**パイプライン**] をクリックします。 ラボのセットアップによっては、パイプラインでアクセス許可の入力が求められる場合があります。 **[許可]** をクリックして、パイプラインの実行を許可します。 
...
2. エージェント プールを削除します。
3. PAT トークンを取り消します。
4. Repos/.ado/eshoponweb-ci-pr.yml ファイルに移動し、**[編集]** を選び、13 から 15 行目 (エージェント プール スニペット) を削除して元の `vmImage: windows-latest` に戻すことで、**eshoponweb-ci-pr.yml** ファイルの変更を元に戻します。 (これは、この後のラボ演習で同じサンプル パイプライン ファイルを使うためです)。 
