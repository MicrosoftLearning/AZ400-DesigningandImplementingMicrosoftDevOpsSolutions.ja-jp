...
1. テンプレートのリストのツール バーで、**[DevOps Labs]** をクリックし、**ReleaseGates** テンプレートを選択して、**[テンプレートの選択]** をクリックします。
...
1. 変更が有効になるまで待ち、[Application Insights] ウィンドウで **[Application Insights データの表示]** リンクをクリックします。
...
1.  **[アラート ルールの作成]** ウィンドウの **[条件]** セクションで、**[条件の追加]** リンクをクリックします。 
1.  **[Select a signal]\(シグナルの選択\)** ブレードの **[シグナル名で検索]** テキストボックスに「**Failed Requests**」と入力して選択します。 
1.  **[シグナル ロジックの構成]** ウィンドウの **[アラート ロジック]** セクションで、**[しきい値]** を **[静的]** に設定したままにし、**[しきい値]** テキストボックスに「**0**」と入力して、**[完了]** をクリックします。

    > **注**:ルールは、過去 5 分間に失敗したリクエストの数が 0 を超えると、アラートを生成します。

1.  **[アラート ルールの作成]** ペインに戻り、 **[詳細]** で次の設定を指定し、情報を入力して (他は既定値のままにします)、 **[確認と作成] > [作成]** をクリックします。
...
1. **[パイプライン]** タブの **[成果物]** 長方形で、**PartsUnlimited-CI** ビルド成果物の右上隅にある **[継続的デプロイ トリガー]** ボタンをクリックします。
...
1.  **[すべてのパイプライン > PartsUnlimited-CD]** ペインで、**[タスク]** タブをクリックし、ドロップダウン リストで "**本番**" を選択します。
1.  **本番** ステージを選択した状態で、**[Azure サブスクリプション]** ドロップダウン リストで **[利用可能な Azure サービス接続]** の下に表示される **カナリア環境** ステージに使用した Azure サブスクリプションを選択します。これは、サブスクリプションの使用を承認する前にサービス接続を既に作成しているためです。
...
1. Azure portal インターフェイスに切り替え、リソース グループ **az400m10l01-RG** に移動し、リソースのリストで "**カナリア**" Web アプリをクリックし、Web アプリ ウィンドウで **[参照]** をクリックして、Web ページが新しい Web ブラウザー タブに正常に読み込まれることを確認します。
1. **Parts Unlimited** Web サイトを表示している Web ブラウザー タブを閉じます。
1. Azure portal インターフェイスに切り替え、リソース グループ **az400m10l01-RG** に移動し、リソースのリストで "**本番**" Web アプリをクリックし、Web アプリ ウィンドウで **[参照]** をクリックして、Web ページが新しい Web ブラウザー タブに正常に読み込まれることを確認します。
1. **Parts Unlimited** Web サイトを表示している Web ブラウザー タブを閉じます。
...
1. **[すべてのパイプライン > PartsUnlimited-CD]** ペインで、**カナリア環境** ステージを表す長方形の左端で、**[配置前の条件]** を表す楕円形をクリックします。
1. **[配置前の条件]** ウィンドウで、**[配置前の承認]** スライダーを **[有効]** に設定し、**[承認者]** テキスト ボックスに Azure DevOps アカウント名を入力して選択します。
1. **[配置前の条件]** ペインで、**[ゲート]** スライダーを **[有効]** に設定し、**[+ 追加]** をクリックして、ポップアップ メニューで **[作業項目のクエリ]** をクリックします。
1. **[配置前の条件]** ペインの **[作業項目のクエリ]** セクションの **[クエリ]** ドロップダウン リストで、**[共有クエリ]** の下の **[バグ]** を選択し、**[上限しきい値]** の値を **0** のままにします。
...
1. **[配置前の条件]** ペインで、**[評価オプション]** セクションを展開し、次のオプションを構成します
...
1. **[配置前の条件]** ペインで、**[正常に実行されたゲートで、承認を求めます]** ラジオ ボタンを選択します。
1. 右上隅の **[x]** マークをクリックして、**[配置前の条件]** ペインを閉じます。 リリース パイプラインの変更を **保存します**。
...
1. **[共有クエリのアクセス許可]** ペインの **[ユーザーまたはグループの検索]** フィールドに、「**リリース ゲートを使用したデプロイの制御ビルド サービス**」 (<プロジェクト名> ビルド サービス) と入力するか貼り付けて、見つかった ID をクリックします。
...
1. **[ユーザー]** の一覧でユーザー "**リリース ゲートを使用したデプロイの制御ビルド サービス**" を選択し、右側のサイトで **[読み取り]** アクセス許可を **[許可]** に設定します。
...
1.  **[すべてのパイプライン > PartsUnlimited-CD]** ペインに戻り、**カナリア環境** ステージを表す長方形の右端で、**配置後の条件** を表す楕円形をクリックします。
...
1. "**カナリア環境でディスクの空き容量**" エントリをクリックします。
1. "**カナリア環境でディスクの空き容量**" のペインで、左上隅の **[状態]** ラベルの横にある **[新規]** をクリックし、ドロップダウン リストで **[閉じる]** をクリックして、**[保存]** をクリックします。
1. **[カナリア環境]** ペインに戻り、2 番目の評価が合格するのを待ちます。

    > **注**: 2 番目の評価が既に失敗した場合は、"**カナリア環境**" ステージを表す長方形をポイントして **[再デプロイ]** オプションを表示し、**[再デプロイ]** をクリックし、**[カナリア環境]** ウィンドウで、**[デプロイ]** をクリックし、デプロイ前のゲートの処理状態を監視します。
...
1. 失敗した要求をトリガーするには、Azure portal を表示している Web ブラウザー ウィンドウに切り替え、"**カナリア**" Azure Web アプリ ウィンドウに移動して、**[参照]** をクリックします。 これにより、PartsUnlimited Web サイトを表示する新しいブラウザー タブが開きます。
...
1. "**カナリア**" Web アプリ ページの Application Insights ウィンドウに移動して、失敗した要求が Application Insights によって検出されたことを検証し、Application Insights ウィンドウで **[アラート]** をクリックして、ページの一覧に 1 つ以上の **Sev 2** アラートが表示されていることを確認します。
