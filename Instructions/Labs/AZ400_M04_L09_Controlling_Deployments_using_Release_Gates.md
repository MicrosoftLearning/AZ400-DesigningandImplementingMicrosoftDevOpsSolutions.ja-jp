...
1.  **[Select a signal] (シグナルの選択)** ブレードの **[シグナル名で検索]** テキストボックスに「**要求**」と入力します。 結果の一覧から **[失敗した要求]** を選びます。 
...
1. **[パイプライン]** タブで、 **[成果物の追加]** 四角形を選び、 **[ソース (ビルド パイプライン)]** フィールドで **eShopOnWeb** を選びます。 **[追加]** をクリックして、成果物の選択を確認します。
...
1. **[パイプライン]** ペインで、**eShopOnWeb** ビルド パイプラインを表すエントリをクリックしてから、**eShopOnWeb** のペインで **[パイプラインの実行]** をクリックします。
...
1. 垂直ナビゲーション ウィンドウの **[パイプライン]** セクションで **[リリース]** をクリックし、**eShopOnWeb** のペインで最新のリリースを表すエントリをクリックします。
1. **eShopOnWeb > Release-1** のブレードで、リリースの進行状況を追跡し、両方の Web アプリへのデプロイが正常に完了したことを確認します。
...
1. Azure Portal の [リソース、サービス、ドキュメントを検索します] フィールドに「**Application Insights**」と入力し、前の演習で作成した **Canary-AppInsights** リソースを選びます。 次に、 **[アラート]** に移動します。 
...
このタスクでは、Azure Cloud Shell を使用して、このラボでプロビジョニングされた Azure リソースを削除し、不要な料金を削減します。
