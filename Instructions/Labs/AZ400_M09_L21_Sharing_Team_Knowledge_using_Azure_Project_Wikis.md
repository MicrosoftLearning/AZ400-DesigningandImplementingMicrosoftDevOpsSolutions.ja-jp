...
1. Wiki ソース ファイルは、Repos の現在のフォルダー構造内の別のフォルダーに格納します。 **Repos** 内から、 **[ファイル]** を選びます。 フォルダー構造の上に **EShopOnWeb** という Repo タイトルがあることに注意してください。 **省略記号 (3 つのドット) を選び**、**[新規] > [フォルダー]** を選んで、[新しいフォルダー名] のタイトルとして「**ドキュメント**」を指定します。 リポジトリでは空のフォルダーを作成できないため、[新しいファイル名] として「**READ.ME**」を指定します。 
