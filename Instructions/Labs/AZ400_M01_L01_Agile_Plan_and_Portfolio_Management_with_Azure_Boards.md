...
1. ページの左下隅にある **[プロジェクト設定]** というラベルの付いた歯車アイコンをクリックして、**[プロジェクト設定]** ページを開きます。
    ...
    ![Azure DevOps の設定ページのスクリーンショット。](images/m1/project_settings_v1.png) 

1. **[全般]** セクションで、**[チーム]** タブを選択してください。このプロジェクトには既定のチーム (**eShopOnWeb チーム**) が既にありますが、このラボ用に新しく作成します。 **[新しいチーム]** をクリックします。

    ![[チームのプロジェクトの設定] ウィンドウのスクリーンショット。](images/m1/new_team_v1.png)

1. **[新しいチームの作成]** ペインの **[チーム名]** テキストボックスに「**`EShop-Web`**」と入力し、他の設定を既定値のままにして、**[作成]** をクリックします。

    ![[プロジェクト設定チーム] ページのスクリーンショット。](images/m1/eshopweb-team_v1.png)
...
1. **[EShop-Web]** ページの上部にある **[イテレーションと区分パス]** リンクをクリックして、チームのスケジュールとスコープの定義を始めます。
    ...
    ![[イテレーションと区分パス] オプションのスクリーンショット。](images/m1/EShop-WEB-iterationsareas_v1.png)

1. **[ボード]** ペインの上部にある **[イテレーション]** タブを選択し、**[イテレーションの選択]** をクリックします。

    ![イテレーション構成ページのスクリーンショット。](images/m1/EShop-WEB-select_iteration_v1.png)
...
1. **[スプリント 1]** を選び、**省略記号 [...]** をクリックします。コンテキスト メニューの **[編集]** を選びます。
     ...
     ![イテレーション タブ編集設定のスクリーンショット。](images/m1/EShop-WEB-edit_iteration_v1.png)
...
1. 前の手順を繰り返して、**スプリント 2** と**スプリント 3** を追加します。 現在は、最初のスプリントの第 2 週ということになります。
    ...
    ![[イテレーション] タブの下のスプリント構成のスクリーンショット。](images/m1/EShop-WEB-3sprints_v1.png)

1. 引き続き **[プロジェクトの設定] > [ボード] > [チームの構成]** ペインで、ペインの上部にある **[エリア]** タブを選びます。チームの名前と一致する名前の自動生成されたエリアがあります。

    ![区分設定ページのスクリーンショット。](images/m1/EShop-WEB-areas_v1.png)

1. **[既定の区分]** エントリの横にある省略記号 [...] をクリックして、ドロップダウン リストで **[サブ区分を含む]** を選びます。

    ![[区分] タブの下の [サブ区分を含める] オプションのスクリーンショット。](images/m1/EShop-WEB-sub_areas_v1.png)
...
1. Azure DevOps ポータルの左上隅にあるプロジェクト名をクリックして、プロジェクトのホーム ページに戻ります。
...
1. **[作業項目] ** ウィンドウで、**[新しい作業項目] > [エピック] ** の順にクリックします。
    ...
    ![作業項目の作成のスクリーンショット。](images/m1/EShop-WEB-create_epic_v1.png)

1. **[タイトルの入力]** テキストボックスに、「**`Product training`**」と入力します。
1. 左上隅にある **[未選択]** エントリを選択し、ドロップダウン リストでユーザー アカウントを選択して、新しい作業項目を自分に割り当てます。 名前の先頭が表示されない場合は、名前の入力を開始し、**[検索]** をクリックします。
...
1. **[保存]** をクリックして、変更を確定します。 **閉じないでください**。
    ...
    ![作業項目の詳細 (タイトル、区分、イテレーション オプションなど) のスクリーンショット。](images/m1/EShop-WEB-epic_details_v1.png)
...
1. **[リンクの追加]** パネルの **[リンクの種類]** ドロップダウン リストで、 **[子]** を選びます。 次に、**[作業項目の種類]** ドロップダウン リストで **[機能]** を選び、**[タイトル]** テキストボックスに「**`Training dashboard`**」と入力します。
    ...
    ![作業項目リンクの作成のスクリーンショット。](images/m1/EShop-WEB-create_child_feature.png)

1. **[リンクの追加]** をクリックして、子項目を保存します。

    ![作業項目に関連する作業区分のスクリーンショット。](images/m1/EShop-WEB-epic_with_linked_item_v1.png)

    > **注**: **[トレーニング ダッシュボード]** パネルで、割り当て、**エリア**、**イテレーション**が、機能の基になっているエピックと同じ値に既に設定されていることに注意してください。 さらに、この機能は、作成元の親アイテムに自動的にリンクされます。

1. (新機能) **[トレーニング ダッシュボード]** パネルで、**[保存して閉じる]** をクリックします。

1. Azure DevOps ポータルの垂直ナビゲーション ウィンドウの **[ボード]** 項目の一覧で、 **[ボード]** を選びます。
1. **[ボード]** パネルで、 **[EShop-WEB ボード]** エントリを選びます。 これにより、その特定のチームのボードが開きます。

    ![EShop-WEB ボードの選択のスクリーンショット。](images/m1/EShop-WEB-_boards_v1.png)
...
1. 省略記号アイコン [...] をクリックして、ドロップダウン リストで **[プロダクト バックログ項目の追加]** を選びます。
    ...
    ![トレーニング ダッシュボード機能の [プロダクト バックログ項目の追加] オプションのスクリーンショット。](images/m1/EShop-WEB-add_pb_v1.png)
...
1. 前の手順を繰り返して、顧客が最近表示したチュートリアルを表示し、**`As a customer, I want to see tutorials I recently viewed`** と **`As a customer, I want to request new tutorials`** という名前の新しいチュートリアルを要求できるように設計された 2 つの PBI を追加します。
    ...
    ![プロダクト バックログの追加のスクリーンショット。](images/m1/EShop-WEB-pbis_v1.png)

1. **[ボード]** パネルの右上隅にある **[機能]** エントリを選択し、ドロップダウン リストで **[バックログ項目]** を選択します。

     ![バックログ項目の表示のスクリーンショット。](images/m1/EShop-WEB-backlog_v1.png)
...
1. **[EShop-WEB]** パネルの **[ボード]** タブで、 **[As a customer, I want to view new tutorials] (顧客として、新しいチュートリアルを表示する)** という名前の最初の作業項目を **[新規]** から **[承認済み]** ステージにドラッグします。
    ...
    ![作業項目を含むボードのスクリーンショット。](images/m1/EShop-WEB-new2ap_v1.png)
...
1. **[EShop-WEB]** パネルの **[ボード]** タブで、 **[As a customer, I want to request new tutorials] (顧客として、新しいチュートリアルを要求する)** という名前の 3 番目の作業項目を **[新規]** から **[完了]** ステージにドラッグします。
    ...
    ![前の手順で指定した列に作業項目が移動されたボードのスクリーンショット。](images/m1/EShop-WEB-board_pbis_v1.png)

    > **注**:タスク ボードは、バックログの 1 つのビューです。 表形式のビューを使用することもできます。

1. **[EShop-WEB]** ペインの **[ボード]** タブで、ペインの上部にある **[バックログとして表示]** をクリックして、表形式を表示します。

    ![EShop-WEB バックログのスクリーンショット。](images/m1/EShop-WEB-view_backlog_v1.png)
...
1. **[EShop-WEB]** ペインの **[バックログ]** タブのペインの左上隅で、最初の作業項目の横にあるプラス記号をクリックします。 これにより、**[新しいタスク]** パネルが表示されます。
    ...
    ![バックログ リストの [タスクの作成] オプションのスクリーンショット。](images/m1/new_task_v1.png)
...
1. **[新しいタスク]** パネルで、**[保存して閉じる]** をクリックします。
    ...
    ![新しい作業項目の作成のスクリーンショット。](images/m1/EShop-WEB-save_task_v1.png)

1. 最後の 5 つの手順を繰り返して、**`Optimize data query for most recent tutorials`** という名前の別のタスクを追加します。 **残りの作業**を **3** に設定し、**アクティビティ**を**設計**に設定します。 完了したら、**[保存して閉じる]** をクリックします。
...
1. **[スプリント]** ビューの **[タスクボード]** タブのツールバーの右側で、**[表示オプション]** 記号 (目標到達プロセス アイコンのすぐ左) を選択し、**[表示オプション]** ドロップダウン リストで **[作業の詳細]** エントリを選択します。 フィルターとして **[スプリント 2]** を選びます。
    ...
    ![作業の詳細を含む EShop-WEB ボードのスクリーンショット。](images/m1/EShop-WEB-work_details_v1.png)
...
1. **[スプリント]** ビューの **[容量]** タブを選択します。
    ...
    ![容量時間付き作業項目を含むボードのスクリーンショット。](images/m1/EShop-WEB-capacity_v1.png)
...
1. **[スプリント]** ビューの **[容量]** タブで、自分のユーザー アカウントについて、**"アクティビティ"** フィールドを **[開発]** に設定し、**[1 日あたりの容量]** ボックスに「**1**」と入力します。 **[保存]** をクリックします。
    ...
    ![容量構成のスクリーンショット。](images/m1/EShop-WEB-capacity-setdevelopment_v1.png)
...
1. 表示されたパネルで、カレンダー ビューを使用して、現在のスプリント中 (次の 3 週間以内) に 5 営業日にわたる休暇を設定し、完了したら **[OK]** をクリックします。
    ...
    ![休日構成のスクリーンショット。](images/m1/EShop-WEB-days_off_v1.png)

1. **[スプリント]** ビューの **[容量]** タブに戻り、**[保存]** をクリックします。
1. **[スプリント]** ビューの **[タスクボード]** タブを選択します。

    ![[作業の詳細] セクションのスクリーンショット。](images/m1/EShop-WEB-work_details_window_v1.png)
...
1. **[スプリント]** ビューの **[タスクボード]** タブの、 **[Add page for most recent tutorials] (最新のチュートリアルの追加)** を表す正方形のボックスで、前のステップで確認したこのスプリントの合計容量に一致する **14** に、推定時間数を設定します。
    ...
    ![ボードとチームに割り当てられた時間容量のスクリーンショット。](images/m1/EShop-WEB-over_capacity_v1.png)
...
1. **[スプリント]** ビューの **[タスクボード]** タブのツール バーの右側で、**[表示オプション]** 記号 (ファネル アイコンのすぐ左) を選び、**[表示オプション]** ドロップダウン リストで **[割り当て先]** エントリを選びます。
...
1. **[設定]** パネルで、**[スタイル]** タブを選択し、**[スタイル ルールの追加]** をクリックします。**[ルール名]** ラベルの下の **[名前]** テキストボックスに「**`Development`**」と入力し、**[色]** ドロップダウン リストで緑色の長方形を選択します。

    > **注**: これにより、ルール名のすぐ下で設定したルール基準を満たす場合、すべてのカードが緑色になります。

1. ルール名の下のセクションで、**[フィールド]** ドロップダウン リストから **[アクティビティ]** を選択し、**[オペレーター]** ドロップダウン リストで **[=]** を選択し、**[値]** ドロップダウン リストで **[開発]** を選択します。

    ![ボード スタイル設定のスクリーンショット。](images/m1/EShop-WEB-styles_v2.JPG)

1. **[保存]** をクリックし、設定を保存して閉じます。

    ![チーム ボードのタスク スタイルのスクリーンショット。](images/m1/EShop-WEB-sprint-green_v1.png)

    > **注**:これにより、**開発**アクティビティに割り当てられたすべてのカードが緑色に設定されます。

1. **[設定]** パネルに戻って、**[全般]** タブを選択し、**[バックログ]** セクションでナビゲーション レベルを表示し、構成します。
...
1. **[設定]** パネルで、**[全般]** タブを選択すると、**[バグの作業]** セクションで、バックログとボードでバグを管理する方法を指定できます。
...
1. **[設定]** パネルで、**[タグの色]** タブを選択し、**[タグの色を追加]** をクリックして、**[タグ]** テキストボックスに「**`data`**」と入力し、黄色の四角形を選択します。

    ![タグ設定のスクリーンショット。](images/m1/EShop-WEB-tag_color_v1.png)
...
1. **[保存して閉じる]** をクリックして、これらの編集を保存します。

    ![カードに表示されている 2 つの新しいタグのスクリーンショット。](images/m1/EShop-WEB-tags_v1.png)
...
1. **[列名]** ラベルの下の **[列の追加]** をクリックし、**[名前]** テキストボックスに「**`QA Approved`**」と入力し、**[WIP 制限]** テキストボックスに「**1**」と入力します。

    > **注**:1 は、この段階で一度に 1 つの作業項目のみを含める必要があることを示します。 通常はこれを高く設定しますが、機能を示すための作業項目は 2 つだけです。

    ![WIP 設定のスクリーンショット。](images/m1/EShop-WEB-qa_column_v1.png)
...
1. **[顧客として新しいチュートリアルを表示する]** 作業項目を **[承認済み]** ステージから **[QA 承認済み]** ステージにドラッグします。
    ...
    ![WIP 制限を超えるボードのスクリーンショット。](images/m1/EShop-WEB-wip_limit_v1.png)
...
1. **[保存]** をクリックし、設定を保存して閉じます。
    ...
    ![設定分割列と完了の定義の構成のスクリーンショット。](images/m1/dd_v1.png)
    ...
    ![QA 承認済み列の分割列のスクリーンショット。](images/m1/EShop-WEB-qa_2columns_v1.png)
...
1. **[スイムレーン]** タブで、**[スイムレーン名]** ラベルのすぐ下にある **[スイムレーンの追加]** をクリックし、**[名前]** テキストボックスに「**`Expedite`**」と入力します。
1. **[色]** ドロップダウン リストで、**緑色**の四角形を選択します。
1. **[保存]** をクリックし、設定を保存して閉じます。

    ![スイムレーンの迅速な作成のスクリーンショット。](images/m1/EShop-WEB-swimlane_v1.png)
...
この演習では、かんばんボードを使用して、作業の流れを理解しやすく、進行状況を追跡しやすいように視覚化する方法を説明しました。 また、チームのプロセスをサポートするようにボードを構成する方法と、進行中の作業 (WIP) の制限を設定して、チームが作業量に圧倒されないようにする方法についても説明しました。

### 演習 2: ダッシュボードを定義する

#### タスク 1: ダッシュボードを作成してカスタマイズする
...
1. **eShopOnWeb チーム**の **[概要]** を選択し、既存のダッシュボードを確認します。
    ...
    ![ダッシュボードの概要ページのスクリーンショット。](images/m1/EShop-WEB-dashboard_v1.png)

1. **[ダッシュボード]** ペインで、右上隅の **[+ 新しいダッシュボード]** を選択します。

1. **[ダッシュボードの作成]** ペインの **[名前]** テキストボックスに「**`Product training`**」と入力し、**[チーム]** ドロップダウン リストで、**EShop-WEB** チームを選んで、**[作成]** をクリックします。

    ![ダッシュボード パネルの作成のスクリーンショット。](images/m1/EShop-WEB-create_dash_v1.png)

1. 新しいダッシュボードのペインで、**[ウィジェットの追加]** をクリックします。
1. **[ウィジェットの追加]** パネルで、**[ウィジェットの検索]** テキストボックスに「**`sprint`**」と入力して、スプリントに焦点を当てた既存のウィジェットを検索します。 結果のリストで、**[スプリントの概要]** を選択し、**[追加]** をクリックします。
1. 新しく追加されたウィジェットを表す長方形で、**[設定]** 歯車アイコンをクリックし、**[構成]** ペインを確認します。

    > **注**:カスタマイズ レベルはウィジェットによって異なります。

1. **[構成]** ペインで、変更を加えずに **[閉じる]** をクリックします。
1. **[ウィジェットの追加]** ペインに戻り、**[検索]** テキストボックスにもう一度「**`sprint`**」と入力して、スプリントに焦点を当てた既存のウィジェットを検索します。 結果のリストで、**[スプリント容量]** を選択し、**[追加]** をクリックします。

    > **注**: ウィジェットに「スプリント容量ウィジェットを使用するための容量を設定してください」と表示されている場合は、**[容量の設定]** リンクを選択して容量を設定できます。 [アクティビティ] を [開発] に設定し、[容量] を 1 に設定します。 **[保存]** をクリックし、ダッシュボードに戻ります。
    
1. **[ダッシュボード]** ビューのペインの上部にある **[編集の完了]** をクリックします。

    ![2 つの新しいウィジェットを含むダッシュボードのスクリーンショット。](images/m1/EShop-WEB-finished_dashboard_v1.png)
...
1. **[新しい句の追加]** をクリックし、**[フィールド]** 列で **[区分パス]** を選択し、対応する **[値]** ドロップダウン リストで、**eShopOnWeb\\EShop-WEB** を選択します。
1. **[保存]** をクリックします。

    ![クエリ エディターの新しいクエリのスクリーンショット。](images/m1/EShop-WEB-query_v1.png)

1. **[名前の入力]** テキストボックスに「**`Web tasks`**」と入力し、**[フォルダー]** ドロップダウン リストで **[共有クエリ]** を選択して、**[OK]** をクリックします。
1. **[クエリ]、[共有クエリ]** ペインで、**[Web タスク]** をクリックしてクエリを開きます。
1. **[グラフ]** タブを選択し、**[新しいグラフ]** をクリックします。
1. **[グラフの構成]** パネルで、**[名前]** テキストボックスに「**`Web tasks - By assignment`**」と入力し、**[グループ化]** ドロップダウン リストで **[割り当て先]** を選択してから、**[グラフの保存]** をクリックして変更を保存します。

    ![新しい Web タスクの円グラフのスクリーンショット。](images/m1/EShop-WEB-chart_v1.png)

    > **注**:これで、このグラフをダッシュボードに追加できます。

1. **[概要]** メニューの **[ダッシュボード]** セクションに戻ります。 **EShop-Web** セクションで、前に使った **[製品トレーニング]** ダッシュボードを選んで開きます。

1. 上部のメニューから **[編集]** をクリックします。 **[ウィジェットの追加]** の一覧で「**`Chart`**」を検索し、**[作業項目のグラフ]** を選択します。 **[追加]** をクリックして、このウィジェットを EShop-Web ダッシュボードに追加します。
...
このラボでは、Azure Boards を使用して、チーム、エリア、イテレーション、作業項目、スプリントと容量の管理、かんばんボードのカスタマイズ、ダッシュボードの定義など、多くの一般的なアジャイル計画とポートフォリオ管理タスクを実行しました。
