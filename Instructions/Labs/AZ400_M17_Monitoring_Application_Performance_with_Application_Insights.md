...
1. 次に、Application Insights インスタンスを作成します。
    ...
    > **注**: 「コマンドには拡張機能 application-insights が必要です。今すぐインストールしますか?」というプロンプトが表示された場合は、Y と入力し、Enter キーを押します。

1. Application Insights を Web アプリケーションに接続しましょう。
...
1. 「**アプリケーション設定**」 リストで 「**APPINSIGHTS_INSTRUMENTATIONKEY**」 エントリをクリックします。(このエントリが表示されない場合は、**Application Insights** を選択し、設定の下で、**「Aplication Insights の有効化」** を選択してから、**「適用」** を選択します。) 
...
1.  その結果、「**Application Insights**」 ブレードには収集されたデータのさまざまな特徴を示す図が表示されるので、これを確認します。この中には、生成されたトラフィックや、このタスクで先ほどトリガーされたものの失敗した要求が含まれます。
    ...
    > **注**: すぐに何も表示されない場合は、数分待ってから、概要セクションにログが表示されるまでページを更新してください。
