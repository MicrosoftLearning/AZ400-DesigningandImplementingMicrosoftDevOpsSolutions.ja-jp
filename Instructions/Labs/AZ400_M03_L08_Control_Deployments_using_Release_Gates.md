...
1. **Bash** または **PowerShell** の選択を求めるメッセージが表示されたら、**[Bash]** を選択します。
   ...
   > **注**:**Cloud Shell** の初回起動時に「**ストレージがマウントされていません**」というメッセージが表示された場合は、このラボで使用しているサブスクリプションを選択し、**[適用]** をクリックします。
...
1. ユニークなアプリ名を持つ 2 つの Web アプリを作成します。
   ...
   > **注**:"サブスクリプションは名前空間 'Microsoft.Web' を使用するように登録されていません" というエラーが発生した場合は、次のコマンドを実行して Microsoft.Web リソース プロバイダー`az provider register --namespace Microsoft.Web` を登録し、登録が完了するのを待ってから Web アプリを作成します。
...
1. Azure DevOps ポータルの **eShopOnWeb** プロジェクトから、垂直ナビゲーション ウィンドウの **[パイプライン]** を選び、 **[パイプライン]** セクション内の **[リリース]** をクリックします。
   > **注**:**[リリース]** セクションが表示されない場合は、**[プロジェクトの設定]** > **[パイプライン]** > **[設定]** > **[全般]** セクションで **[クラシック リリース パイプラインの作成を無効にする]** トグルが **[オン]** に設定されていることが原因である可能性があります。
...
1. **[Apply]** をクリックします。
1. 表示された **[ステージ]** ウィンドウから、既定の "Stage 1" というステージ名を **`DevTest`** に更新します。 **[X]** ボタンを使ってポップアップ ウィンドウを閉じます。 これで、リリース パイプラインのグラフィカル エディターに移動し、DevTest ステージが表示されます。
1. ページの上部で、現在のパイプラインの名前を **New release pipeline** から **`eshoponweb-cd`** に変更します。
1. DevTest ステージの上にマウス ポインターを置き、 **[複製]** ボタンをクリックして DevTest ステージを追加のステージにコピーします。 このステージの名前を **`Production`** に設定します。
